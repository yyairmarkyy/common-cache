...
public class ImageManagerLru {
	...
	public static ImageManagerLru from(Context context) {
		...
		if (imageManager == null) {
			imageManager = new ImageManagerLru(myapp);
		}
		return imageManager;
	}
	...
	public void displayImage(ImageView imageView, String url, int resId, boolean isScale) {
		if (imageView == null) {
			return;
		}
		if (imageView.getTag() != null && imageView.getTag().toString().equals(url)) {
		...
		queueImage(new ImageRef(imageView, url, filePath, resId, isScale));
	}
			...
			case MSG_REQUEST: // 收到请求
				...
				if (msg.obj != null && msg.obj instanceof ImageRef) {
					...
					if (StringUtils.isEmpty(url))
						return;
					bitmap = mDiskCache.get(url);// 2.如果存在sd 磁盘缓存中
					if (bitmap != null) {
						// 写入map缓存
						...
						try {
							byte[] data = loadByteArrayFromNetwork(url);// 3.从网络获取数据
							if (data != null) {
